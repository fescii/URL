# Scale Analysis Report: 3,000,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **3,000,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `3,000,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `564855.84 KB` (578412378 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `508041.20 KB` (520234186 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.06%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `83850666 B` (~28.0 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `11907246 B` (**3.97 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,266 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `3.06 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 3,000,000 sample lookups matched byte-for-byte).
