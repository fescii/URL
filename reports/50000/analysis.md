# Scale Analysis Report: 50,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **50,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `50,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `9308.03 KB` (9531420 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `8351.76 KB` (8552207 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.27%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `2937958 B` (~58.8 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `476389 B` (**9.53 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,241 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `7.91 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 50,000 sample lookups matched byte-for-byte).
