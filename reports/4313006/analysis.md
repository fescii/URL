# Scale Analysis Report: 4,313,006 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **4,313,006 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `4,313,006` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `812879.87 KB` (832388982 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `731107.71 KB` (748654296 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.06%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `106367574 B` (~24.7 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `15634045 B` (**3.62 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,224 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `111.93 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 4,313,006 sample lookups matched byte-for-byte).
