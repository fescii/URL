# Scale Analysis Report: 1,000,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **1,000,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `1,000,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `187630.60 KB` (192133734 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `168674.96 KB` (172723160 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.10%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `40735412 B` (~40.7 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `5953689 B` (**5.95 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,309 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `17.08 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 1,000,000 sample lookups matched byte-for-byte).
