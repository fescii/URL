# Scale Analysis Report: 1,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **1,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `1,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `183.95 KB` (188365 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `150.84 KB` (154459 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `18.00%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `72000 B` (~72.0 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `11907 B` (**11.91 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `1,209 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `6.75 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 1,000 sample lookups matched byte-for-byte).
