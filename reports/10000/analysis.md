# Scale Analysis Report: 10,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **10,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `10,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `1858.68 KB` (1903289 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `1651.88 KB` (1691520 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `11.13%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `407446 B` (~40.7 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `59542 B` (**5.95 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,102 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `5.65 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 10,000 sample lookups matched byte-for-byte).
