# Scale Analysis Report: 500,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **500,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `500,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `93700.44 KB` (95949247 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `84208.53 KB` (86229538 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.13%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `29375564 B` (~58.8 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `4763348 B` (**9.53 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,286 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `3.35 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 500,000 sample lookups matched byte-for-byte).
