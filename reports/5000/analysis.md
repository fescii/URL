# Scale Analysis Report: 5,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **5,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `5,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `931.84 KB` (954207 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `819.36 KB` (839028 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `12.07%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `299547 B` (~59.9 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `47590 B` (**9.52 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,094 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `2.91 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 5,000 sample lookups matched byte-for-byte).
