# Scale Analysis Report: 2,000,000 URLs

This document provides in-depth technical analysis for the ingestion and storage benchmark of **2,000,000 URLs**.

## 1. Key Performance Metrics

| Metric | Value | Description |
|:---|:---|:---|
| **Total Processed URLs** | `2,000,000` | Ingested from real-world `list.csv` |
| **Ingested Raw Volume** | `376027.21 KB` (385051868 B) | Uncompressed UTF-8 input size |
| **Stored Disk Footprint** | `338161.54 KB` (346277415 B) | Append-only sharded Bitcask logs |
| **Disk Savings Ratio** | `10.07%` | Net storage reduction vs raw URLs |
| **In-Memory Index (Mutable)** | `77898249 B` (~38.9 B/key) | Active hash index during ingestion |
| **In-Memory Index (Sealed)** | `11907474 B` (**5.95 B/key**) | Minimal Perfect Hash (MPHF) bitvectors |
| **Ingestion Throughput** | `2,302 URLs/sec` | Batch processing pipeline rate |
| **Average Lookup Latency** | `3.17 µs` | Direct zero-copy mmap point lookup |
...
- **Lossless Integrity**: 100.00% Verified (All 2,000,000 sample lookups matched byte-for-byte).
